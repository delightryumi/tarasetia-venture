...
export async function POST(req: NextRequest) {
  try {
    ...
    for (const tx of transactions) {
      ...
      // 1. Resolve active shift if available
      ...
      try {
        ...
        console.error('Failed to query/append to active shift in Firestore:', err);
      }

      // 2. Build or update pos_orders without overwriting rich data
      const posOrderRef = doc(getHotelCollection(db, 'pos_orders', hotelId), tx.id);
      const existingPosOrder = await getDoc(posOrderRef);
      if (existingPosOrder.exists()) {
        const existingData = existingPosOrder.data();
        if (!existingData.shiftId && resolvedShiftId) {
          await updateDoc(posOrderRef, { shiftId: resolvedShiftId });
        }
      } else {
        const orderData = {
          transactionId: tx.id,
          ...
          cashAmount: (tx.paymentMethod || 'cash') === 'cash' ? Number(tx.totalAmount) : 0,
          changeAmount: 0,
          customerName: 'Guest',
          cashierName: 'Kasir',
          tableNumber: '-',
      ...
      // 3. Write to revenue_transactions if not exists
      const revTxRef = doc(getHotelCollection(db, 'revenue_transactions', hotelId), tx.id);
      const existingRevTx = await getDoc(revTxRef);
      if (!existingRevTx.exists()) {
