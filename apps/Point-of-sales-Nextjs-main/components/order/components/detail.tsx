...
import { syncUnsyncedTransactions } from '@/lib/dexie-sync';
...
useEffect(() => {
  const fetchShopData = async () => {
    try {
      ...
      if (!isOnline) {
        const cachedTax = localStorage.getItem('restoTax');
        if (cachedTax) {
          setTaxRate(parseFloat(cachedTax));
        }
        return;
      }
      ...
      if (response.status === 200) {
        ...
        localStorage.setItem('restoTax', shopdata.tax.toString());
      }
    } catch (error: any) {
      console.error('Failed to fetch shop tax config:', error);
    }
  };
...
const handleCheckout = async () => {
  ...
  try {
    ...
    if (isOnline) {
      ...
      toast.warn('Anda sedang offline. Transaksi disimpan lokal dan akan disinkronkan otomatis saat online kembali.');
    }

    // 4. Clear transaction workspace
