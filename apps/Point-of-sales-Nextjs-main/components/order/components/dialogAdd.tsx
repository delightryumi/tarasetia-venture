...
export function DialogAdd({
      ...
      const fetchProductStocks = async () => {
        try {
          ...
          if (error instanceof Error) {
            ...
            toast.error('An unknown error occurred while fetching product stocks');
          }
        }
      };
  ...
  const handleAdd = async () => {
    ...
    try {
      ...
      const sellPrice = selectedResult.Product?.[0]?.sellprice || selectedResult.price || 0;

      // 1. Insert item locally into Dexie
      ...
      // 2. Deduct local stock in Dexie
      await localDb.products.update(selectedResult.id, {
        stock: Math.max(0, selectedResult.stock - qTyNumber),
      });

      // 3. Attempt to save to cloud database if online
      const isOnline = navigator.onLine;
      if (isOnline) {
        try {
          ...
          console.warn('Gagal sinkronisasi produk ke cloud server, akan disinkronkan saat checkout:', syncErr);
        }
      }
